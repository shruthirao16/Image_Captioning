...
dataset_text = r"C:\Users\takka\OneDrive\Desktop\PROJECTS\Image_captioning\Flickr8k_text"
dataset_images = r"C:\Users\takka\OneDrive\Desktop\PROJECTS\Image_captioning\Flickr8k_Dataset"

#we prepare our text data
# filename = dataset_text + "/" + "Flickr8k.token.txt" 

filename = r"C:\Users\takka\OneDrive\Desktop\PROJECTS\Image_captioning\Flickr8k_text\Flickr8k.token.txt"
...
# Ensure your model is defined before running this
# Example: model = YourPretrainedModel()

# def extract_features(directory):
#     features = {}
#     valid_images = ['.jpg', '.jpeg', '.png']  # Valid image formats
...
#         feature = model.predict(image)  # Ensure 'model' is loaded
#         features[img] = feature

#     return features

# dataset_images = r"C:\Users\takka\OneDrive\Desktop\PROJECTS\Image_captioning\Flickr8k_Dataset\Flicker8k_Dataset"  # Update the path
# features = extract_features(dataset_images)

# # Save features to file
# dump(features, open("features.p", "wb"))
# print("✅ Feature extraction completed and saved to features.p")
...
def load_photos(filename):
    ...
    print(f"✅ Loaded train image list. Total images: {len(photos)}")

    # Print first few image names
    print("First 5 images:", photos[:5])
...
def load_clean_descriptions(filename, photos): 
    file = load_doc(filename)
    print(f"✅ Loaded {filename}. Total lines: {len(file.splitlines())}")

    descriptions = {}

    for i, line in enumerate(file.split("\n")):
        words = line.split("\t")  # Use tab (\t) as separator
        if len(words) < 2:
            continue

        image, image_caption = words[0], words[1]  # No need to split on '#'

        if image in photos:
            if image not in descriptions:
                descriptions[image] = []
            desc = f"<start> {image_caption} <end>"
            descriptions[image].append(desc)

            if i < 5:  # Print first few processed descriptions
                print(f"{i}: {image} -> {desc}")
...
filename = r"C:\Users\takka\OneDrive\Desktop\PROJECTS\Image_captioning\Flickr8k_text\Flickr_8k.trainImages.txt"

print("Full path to train images file:", filename)
...
#own
print(f"Total train descriptions: {len(train_descriptions)}")
print("First 5 descriptions:", list(train_descriptions.items())[:5])
